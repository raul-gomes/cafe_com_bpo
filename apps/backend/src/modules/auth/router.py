...
from .schemas import UserCreate, UserResponse, TokenResponse, RefreshTokenRequest, ForgotPasswordRequest, ResetPasswordRequest
...
def update_me(
    ...
    allowed_fields = {"name", "company", "company_name", "company_segment", "company_description"}
...
def forgot_password(data: ForgotPasswordRequest, service: AuthServiceDep):
    ...
    if not user:
        raise HTTPException(status_code=404, detail="Nenhuma conta encontrada com este e-mail.")

    if user.auth_provider != "local":
        raise HTTPException(status_code=400, detail=f"Esta conta usa login com {user.auth_provider.capitalize()}. Faça login pelo {user.auth_provider.capitalize()}.")
...
def oauth_callback(
    ...
    try:
        ...
        frontend_url = settings.cors_origins.split(",")[0]
        return RedirectResponse(url=f"{frontend_url}/auth/callback?token={tokens['access_token']}&refresh_token={tokens['refresh_token']}")
    except Exception as e:
