...
vi.mock('../src/context/AuthContext', () => ({
  useAuth: () => ({ user: { id: 'user-1', name: 'Raul Gomes', email: 'raul@cafe.com' } }),
}))
...
describe('ConversationPage — conversa privada', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('exibe o projeto, o participante e as mensagens', async () => {
    renderPage()

    expect(await screen.findByText('Automação de fluxo fiscal')).toBeInTheDocument()
    expect(screen.getByText(/Conversa privada com Ana Souza/i)).toBeInTheDocument()
    expect(screen.getByText('Aceito! Prazer em colaborar.')).toBeInTheDocument()
  })

  it('envia mensagem e recarrega a conversa', async () => {
    renderPage()

    const textarea = await screen.findByRole('textbox', { name: /Mensagem/i })
