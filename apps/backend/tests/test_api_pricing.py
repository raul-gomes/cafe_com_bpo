...
def get_valid_payload():
    return {
        ...
        "services": [
            {"name": "Validation", "minutes_per_execution": 10.0, "monthly_quantity": 5}
        ],
...
def test_post_calculate_does_not_expose_internal_traceback_on_domain_error(client):
    payload = get_valid_payload()
    with patch(
        "src.modules.pricing.domain.engine.PricingCalculator.calculate_final_price",
        side_effect=ValueError("Domain logical failure"),
    ):
        response = client.post("/pricing/calculate", json=payload)
...
def test_post_calculate_returns_consistent_error_shape(client):
    ...
    assert "detail" in response.json()
    assert isinstance(
        response.json()["detail"], list
    )  # Pydantic validation error is a list
