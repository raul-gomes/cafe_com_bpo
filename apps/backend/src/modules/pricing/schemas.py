...
class OperationContextSchema(BaseModel):
    ...
    tax_rate: Decimal = Field(
        ...
        ge=0,
        lt=1,
        description="Conversão da aliquota do simples/lucro para fator decimal (ex: 0.1 para 10%).",
    )
...
class ServiceItemSchema(BaseModel):
    ...
    minutes_per_execution: Decimal = Field(
        ..., ge=0, description="Tempo médio gasto para cada rotina em minutos."
    )
...
class PricingCalculateRequest(BaseModel):
    ...
    desired_profit_margin: Decimal = Field(
        ...,
        ge=0,
        description="Percentual como fator exigido de lucro na operação (Markup).",
    )
