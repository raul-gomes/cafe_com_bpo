...
export const OrcamentoDetalhadoPage: React.FC = () => {
  ...
  const handleEmail = async () => {
    ...
    const clientEmail = clients.find(c => c.name.trim().toLowerCase() === proposal.client_name.trim().toLowerCase())?.email;
              ...
              {result.breakdown ? (
                ...
                <>
                  ...
                  <div className="detalhe-breakdown__row">
                    ...
                    <span className="detalhe-breakdown__value">x {formatNumber(result.complexity_multiplier, 2) || '1.00'}</span>
                  </div>
                </>
              )}
                  ...
                  {proposal.input_payload?.operation ? (
                    <>
                      ...
                      <div className="ds-input-group">
                          ...
                          <div className="ds-input" style={{ background: 'rgba(255,255,255,0.03)', border: 'none' }}>
                              {safeNumber(proposal.input_payload.operation.people_count * proposal.input_payload.operation.hours_per_month)}h
                          </div>
                      </div>
                      <div className="ds-input-group">
                          <label className="ds-label">Margem Desejada</label>
                          <div className="ds-input" style={{ background: 'rgba(255,255,255,0.03)', border: 'none' }}>
                              {formatNumber(proposal.input_payload.desired_profit_margin * 100, 0)}%
                          </div>
                      </div>
                    </>
                  ) : (
      ...
      {whatsappModalOpen && (
        <div className="modal-overlay" onClick={() => setWhatsappModalOpen(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <p>Digite o número do WhatsApp para {proposal?.client_name}:</p>
            <input
              type="tel"
              className="ds-input"
              value={phoneNumber}
            ...
            <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
              ...
              <button className="ds-btn ds-btn-primary" onClick={handleSendWhatsApp} disabled={phoneNumber.replace(/\D/g, '').length < 10}>
