...
def calculate_pricing_endpoint(
    request: PricingCalculateRequest, service: PricingServiceDep
):
    try:
        log.debug(
            f"🧮 Processando simulação para {request.operation.people_count} colaboradores."
        )
        result = service.calculate_pricing(request)
        log.info(
            f"✅ Precificação concluída: {result.final_price} | Margem: {request.desired_profit_margin}%"
        )
        return result
