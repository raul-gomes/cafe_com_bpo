...
def test_full_user_flow_integration(client):
    ...
    pricing_payload = {
        ...
        "services": [
            {"name": "Audit", "minutes_per_execution": 30.0, "monthly_quantity": 10}
        ],
        "desired_profit_margin": 0.15,
    }
    pricing_resp = client.post("/pricing/calculate", json=pricing_payload)
    ...
    prop_resp = client.post("/proposals/", json=proposal_payload, headers=auth_headers)
    assert prop_resp.status_code == 201
    proposal_id = prop_resp.json()["id"]

    # 5. Listagem de Propostas
    list_resp = client.get("/proposals/", headers=auth_headers)
...
def test_unauthorized_access_to_pricing_endpoints(client):
    ...
    resp = client.post("/proposals/", json=prop_payload)
