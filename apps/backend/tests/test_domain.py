...
def build_dummy_operation(
    total_cost="10000", people_count=2, hours_per_month="160", tax_rate="0.05"
):
...
def test_calculate_cost_per_hour_from_total_cost_people_and_hours():
    op = build_dummy_operation(
        total_cost="10000", people_count=2, hours_per_month="160"
    )
...
def test_calculate_total_service_cost_from_multiple_services():
    ...
    total = PricingCalculator.calculate_total_service_cost(
        [svc1, svc2], cost_per_minute=Decimal("0.52")
    )
    assert total == Decimal("126.00")


def test_calculate_profit_amount_from_margin():
    profit = PricingCalculator.calculate_profit_amount(
        base_cost=Decimal("126.00"), desired_profit_margin=Decimal("1.00")
    )
    assert profit == Decimal("126.00")


def test_calculate_tax_amount_from_tax_rate():
    tax = PricingCalculator.calculate_tax_amount(
        price_before_tax=Decimal("100.00"), tax_rate=Decimal("0.10")
    )
