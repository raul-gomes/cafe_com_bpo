...
down_revision: str | Sequence[str] | None = "e0e12c9be2cd"
...
def upgrade() -> None:
    ...
    op.add_column(
        "activity_templates", sa.Column("routine_type_id", sa.UUID(), nullable=True)
    )
    ...
    op.add_column(
        "template_activities", sa.Column("due_days", sa.Integer(), nullable=True)
    )
...
def downgrade() -> None:
    ...
    op.drop_constraint(
        "fk_activity_templates_routine_type", "activity_templates", type_="foreignkey"
    )
